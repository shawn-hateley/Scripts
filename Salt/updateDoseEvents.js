...
for (var k = 0; k < stationList.length; k++) {
  ...
  var processedUSFile = directory_name + stationName + "/" + stationName + "US_DoseEvent.dat.csv"
  if (stationName == "SSN626") processedUSFile = directory_name + stationName + "/" + stationName + "AS_DoseEvent.dat.csv";
  ...
  if (processedHeader == rawHeader.replace(/["]+/g, '')){ //strip away double quotes from the data before comparing
    var lastProcessedLine = lastLine(processed); //Get the last line of the stationList
